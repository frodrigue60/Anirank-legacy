<div x-data="{ open: @entangle('showModal') }" x-show="open" x-cloak
    class="fixed inset-0 z-[101] overflow-y-auto overflow-x-hidden p-4 md:p-8" role="dialog" aria-modal="true">
    {{-- Backdrop --}}
        ...
        <div class="glass-panel bg-surface-darker/90 rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-[#181825]">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-xl font-bold text-white tracking-tight">Report User Profile</h3>
                </div>
            ...
            <div class="p-6">
                <form wire:submit.prevent="submitReport" class="space-y-6">
                    @error('reportedUserId')
                        <div class="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 text-xs font-bold">
                            Internal Error: User ID missing. Please close and try again.
                        </div>
                    @enderror
                    <div>
                        <label for="report_reason" class="block text-sm font-medium text-white/60 mb-2">Reason for report</label>
                        <div class="relative">
                            <select wire:model.live="reason" id="report_reason"
                                ...
                                <option value="Offensive About Description">Offensive About Description</option>
                                <option value="Inappropriate Avatar/Banner">Inappropriate Avatar/Banner</option>
                    ...
                    <div>
                        <label for="report_content" class="block text-sm font-medium text-white/60 mb-2">Description</label>
                        <textarea wire:model="content" id="report_content" rows="4"
                            ...
                            placeholder="Provide any details to help us investigate the report..."></textarea>
                        @error('content')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
