                        ...
                        @foreach ($reports as $report)
                            <tr class="hover:bg-zinc-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route('admin.song-reports.toggle', $report->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        @if (!$report->status)
                                            <button type="submit"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        @if (Auth::user()->isAdmin() || Auth::user()->isEditor())
                                            <a href="{{ route('admin.song-reports.show', $report->id) }}"
                                            ...
                                            <form action="{{ route('admin.song-reports.destroy', $report->id) }}" method="post"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
