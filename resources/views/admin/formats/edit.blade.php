...
<div class="space-y-6 max-w-2xl mx-auto">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-white tracking-tight">Edit Format</h1>
            <p class="text-zinc-400 mt-1 uppercase text-[10px] font-black tracking-widest">Update properties for {{ $format->name }}</p>
        </div>
    ...
    <div class="bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-3xl overflow-hidden p-6">
        <form action="{{ route('admin.formats.update', $format) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $format->name) }}" required
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $format->slug) }}" required
                ...
                <p class="text-zinc-500 text-[10px] uppercase tracking-wider mt-1">Unique URL identifier (e.g., tv)</p>
                @error('slug')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
