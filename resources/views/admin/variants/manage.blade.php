...
@section('content')
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                <h1 class="text-3xl font-bold text-white tracking-tight">Theme Variants</h1>
                <p class="text-zinc-400 mt-1 uppercase text-[10px] font-black tracking-widest">Versions for
                    {{ $song->title }}</p>
            </div>

            <div class="flex flex-wrap gap-3">
                <a href="{{ route('admin.variants.create', $song->id) }}"
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-zinc-950/50 border-b border-zinc-800">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">Variant
                                    Details
                                </th>
                                <th class="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-widest text-center">
                                    Video Status</th>
                            ...
                            @isset($songVariants)
                                @foreach ($songVariants as $variant)
                                    <tr class="hover:bg-zinc-800/30 transition-colors group">
                                        <td class="px-6 py-4 text-sm font-mono text-zinc-500 text-center">#{{ $variant->id }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex flex-col">
                                                <span
                                                    class="text-sm font-bold text-white">{{ $variant->slug ?? 'Standard' }}</span>
                                                <span
                                                    class="text-[10px] text-zinc-500 font-mono mt-1">{{ $variant->id }}</span>
                                            </div>
                                        </td>
