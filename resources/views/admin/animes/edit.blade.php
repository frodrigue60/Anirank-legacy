    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md::items-center md:justify-between gap-6">
            <div>
                ...
                <p class="text-zinc-400 mt-1 uppercase text-[10px] font-black tracking-widest">{{ $anime->title }}</p>
            </div>
        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="description" id="description" rows="5"
                                ...
                                placeholder="Enter anime synopsis...">{{ old('description', $anime->description) }}</textarea>
                        </div>
                    </div>
                </div>
                            ...
                            <select name="animeStatus" id="statusId"
                                ...
                                @foreach ($animeStatus as $item)
                                    <option value="{{ $item['value'] }}"
                                        {{ old('animeStatus', $anime->status) == $item['value'] ? 'selected' : '' }}>
                                        {{ $item['name'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <select name="year" id="year" required
                            ...
                            @foreach ($years as $year)
                                <option value="{{ $year->id }}"
                                    {{ old('year', $anime->year_id) == $year->id ? 'selected' : '' }}>
                                    {{ $year->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="season" id="season" required
                            ...
                            @foreach ($seasons as $season)
                                <option value="{{ $season->id }}"
                                    {{ old('season', $anime->season_id) == $season->id ? 'selected' : '' }}>
                                    {{ $season->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="format" id="format_id" required
                            ...
                            @foreach ($formats as $format)
                                <option value="{{ $format->id }}" {{ old('format', $anime->format_id) == $format->id ? 'selected' : '' }}>
                                    {{ $format->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        <select name="genres[]" id="genres" multiple
                            ...
                            @foreach ($genres as $genre)
                                <option value="{{ $genre->id }}" {{ (in_array($genre->id, old('genres', $anime->genres->pluck('id')->toArray()))) ? 'selected' : '' }}>
                                    {{ $genre->name }}
                                </option>
                            @endforeach
                        </select>
