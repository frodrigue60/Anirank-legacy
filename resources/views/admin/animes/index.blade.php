...
@section('content')
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                <h1 class="text-3xl font-bold text-white tracking-tight">Anime Inventory</h1>
                <p class="text-zinc-400 mt-1 uppercase text-[10px] font-black tracking-widest">Imported & Manual Anime
                    Entries</p>
            </div>

            <div class="flex flex-wrap gap-3">
                <form action="{{ route('admin.animes.sync.all') }}" method="POST">
                    @csrf
                    <button type="submit"
        ...
        <div class="space-y-6">
            ...
            <div class="bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-3xl shadow-xl overflow-hidden p-6">
                    ...
                    <div x-data="{ open: false }" class="mb-6">
                        ...
                        <div x-show="open" x-collapse x-cloak class="mt-4 space-y-4">
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                {{-- Search API --}}
                                <form action="{{ route('admin.animes.search.animes') }}" method="POST"
                                ...
                                {{-- Seasonal API --}}
                                <form action="{{ route('admin.animes.seasonal.animes') }}" method="POST"
                                    class="bg-zinc-950/30 p-4 rounded-2xl border border-zinc-800/50">
                                    @csrf
                ...
                {{-- Local Search --}}
                <form action="{{ route('admin.animes.index') }}" method="GET" class="relative group">
                        ...
                        <tbody class="divide-y divide-zinc-800/50">
                            @foreach ($animes as $anime)
                                <tr class="hover:bg-zinc-800/30 transition-colors group">
                                    <td class="px-6 py-4 text-sm font-mono text-zinc-500">#{{ $anime->id }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-wrap gap-1.5">
                                            @isset($anime->season->id)
                                                <a href="{{ route('admin.seasons.show', $anime->season->id) }}"
                                            ...
                                            @endisset
                                            @isset($anime->year->id)
                                                <a href="{{ route('admin.years.show', $anime->year->id) }}"
                                                    class="px-2.5 py-1 bg-zinc-800 text-zinc-300 text-[10px] font-bold uppercase rounded-md hover:bg-blue-600 hover:text-white transition-all">
                                                    {{ $anime->year->name }}
                                                </a>
                                            @endisset
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        @if (Auth::user()->isAdmin() || Auth::user()->isEditor())
                                            <form action="{{ route('admin.animes.toggle.status', $anime->id) }}"
                                                method="post">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase border transition-all {{ $anime->status == true ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30 hover:bg-emerald-500 hover:text-white' : 'bg-amber-500/10 text-amber-500 border-amber-500/30 hover:bg-amber-500 hover:text-white' }}">
                                                    <span
                                                        class="material-symbols-outlined text-sm">{{ $anime->status == true ? 'toggle_on' : 'toggle_off' }}</span>
                                                    {{ $anime->status == true ? 'Public' : 'Private' }}
                                                </button>
                                            </form>
                                        @endif
                                    </td>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center justify-end gap-2 shrink-0">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <a href="{{ route('admin.songs.create', ['anime_id' => $anime->id]) }}"
                                                        class="p-2 bg-zinc-800 hover:bg-blue-600 text-zinc-400 hover:text-white rounded-lg transition-all border border-zinc-700 hover:border-blue-500"
                                                        title="Add Song">
                ...
                <div class="bg-zinc-950/50 px-6 py-6 border-t border-zinc-800">
                    <div class="flex flex-col items-center">
                        <div class="text-zinc-500 text-xs mb-4">
                            Showing <span class="text-white font-bold">{{ $animes->firstItem() }}</span> to <span
                                class="text-white font-bold">{{ $animes->lastItem() }}</span> of <span
                                class="text-white font-bold">{{ $animes->total() }}</span> results
                        </div>
