...
@section('content')
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                <h1 class="text-3xl font-bold text-white tracking-tight">{{ $anime->title }}</h1>
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('admin.animes.force.update', $anime->id) }}"
                ...
                <div class="bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-3xl overflow-hidden shadow-xl">
                    <div class="aspect-3/4 relative">
                        ...
                        <div class="absolute bottom-4 left-4 right-4">
                            <span
                                ...
                                {{ $anime->status == 1 ? 'PUBLISHED' : 'DRAFT' }}
                            </span>
                        </div>
                    </div>
                    <div class="p-6 space-y-4">
                        <div class="flex items-center justify-between text-xs border-b border-zinc-800 pb-3">
                            ...
                            <span class="text-white font-mono">{{ $anime->year->name ?? 'N/A' }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs border-b border-zinc-800 pb-3">
                            <span class="text-zinc-500 font-bold uppercase">Season</span>
                            <span class="text-white font-mono">{{ $anime->season->name ?? 'N/A' }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-zinc-500 font-bold uppercase">Songs</span>
                            <span class="text-white font-mono">{{ $anime->songs->count() }} entries</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                @if ($anime->images()->where('type', 'banner')->exists())
                    <div class="bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-3xl p-4 shadow-xl">
                        ...
                        <div class="rounded-2xl overflow-hidden aspect-21/9 border border-zinc-800">
                            <img src="{{ $anime->banner_url }}" alt="" class="w-full h-full object-cover">
                        </div>
                    </div>
                @endif
            </div>
